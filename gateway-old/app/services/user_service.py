...
class UserService:
    ...
    #     if self.__user_repository.find_user_by_email(user_register.email) is not None:
    ...
    #         hased_password=self.__bcrypt_context.hash(user_register.password)
    #     )
        
    # def login_user(self, user_login: UserLoginRequest) -> UserLoginResponse:
    #     user_model = self.__user_repository.find_user_by_email(email=user_login.email)
    #     if user_model is None:
    #         raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Email hoặc mật khẩu không chính xác")
    #     if not self.__bcrypt_context.verify(user_login.password, user_model.hashed_password):
    #         raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Email hoặc mật khẩu không chính xác")
    ...
    #     self.__user_repository.update_user(user_model)
    ...
    #     user_model = self.__user_repository.find_user_by_id(id=id)
